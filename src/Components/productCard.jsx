    ...
    return (
        <div className='flex flex-col cursor-pointer rounded-lg gap-2 mt-5 justify-start items-start w-44 h-auto lg:h-auto lg:w-auto'>
            ...
            <div>
                ...
                <p className='font-semibold'>${data.price}<span className='pl-1 line-through text-gray-500' >${(20 / 100 * Number(`${data.price}`) + Number(`${data.price}`)).toFixed(2)}</span><span className='text-red-500 text-sm pl-1'>-20%</span></p>
            </div>
        </div>
    )
}
